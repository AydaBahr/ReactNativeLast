...
const PostsScreen = ({navigation}) => {
  ...
  useEffect(() => {
    axios.get(`http://${IP}:3500/post/posts`)
...
  }, []);
  ...
  const getCommentTime = (commentTime) => {
    const currentTime = moment.utc();
    const elapsedTimeInSeconds = currentTime.diff(moment.utc(commentTime), 'seconds');
...
useEffect(() => {
  ...
  const handleSubmit = async (item, values) => {
    ...
    const nurseImging="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ51RNLx44ss8Jgk1GHTnGnRTTpojFlUXKyXQ&usqp=CAU"
    // user._id
  ...
  elevation: 2,
      ...
      <Input
        ...
        rightIcon={
          <Icon
            ...
            type="feather"
            color="#000"
            onPress={() => {handleSubmit(item, commentValue);
              setCommentValue("") }}
          />
        }
      />
    ...
    <View style={styles.header}>
        ...
        />
        <View style={[styles.userInfo , {backgroundColor:'#eee' , borderRadius:30 , padding:10 , margin:5 }]}>
        <TouchableOpacity onPress={() => {
  navigation.navigate('البروفايل', { userId: comment.user_id });
}}>
        <Text style={styles.username} >  {comment.nurseName}
 </Text>
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scrollView}>
        <ImageBackground
          ...
          style={styles.image}>
          {posts.length === 0 && <Text style={styles.noDataText}>No data yet</Text>}
        </ImageBackground>
